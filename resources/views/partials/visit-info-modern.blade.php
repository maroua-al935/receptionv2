<div class="mx-auto max-w-5xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <p class="visitx-eyebrow">{{ $eyebrow ?? 'Fiche visite' }}</p>
            <h1 class="page-title">{{ $title }}</h1>
            <p class="page-subtitle">{{ $subtitle }}</p>
        </div>
        <a href="{{ $backUrl }}" class="danger-action inline-flex w-fit items-center gap-2">
    ...
    <section class="panel panel-pad">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="rounded-lg bg-slate-50 p-4">
                ...
                <p class="mt-1 font-semibold text-slate-900">{{ !empty($data[0]->exit_date) ? to_normal_date($data[0]->exit_date) : 'N/d' }}</p>
            </div>
        </div>
    </section>
</div>
