...
@section('body')
    <div class="mx-auto max-w-5xl space-y-6">
        ...
        @if(in_array((int) Auth::guard('web')->user()->profile, [1, 2, 3], true))
        <section class="panel panel-pad">
            <div class="mb-5 flex items-center gap-3">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                @isset($data[0]->subject)
                    <div class="rounded-lg bg-slate-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Objet</p><p class="mt-1 font-semibold text-slate-900">{{ $data[0]->subject }}</p></div>
                @endisset
                @isset($data[0]->service)
                    <div class="rounded-lg bg-slate-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Service visite</p><p class="mt-1 font-semibold text-slate-900">{{ $data[0]->service }}</p></div>
                @endisset
                @isset($data[0]->usrname)
                    <div class="rounded-lg bg-slate-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Personne visitee</p><p class="mt-1 font-semibold text-slate-900">{{ $data[0]->usrname }}</p></div>
                @endisset
                <div class="rounded-lg bg-slate-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Date d'entree</p><p class="mt-1 font-semibold text-slate-900">{{ to_normal_date($data[0]->entry_date) }}</p></div>
                <div class="rounded-lg bg-slate-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Date de sortie</p><p class="mt-1 font-semibold text-slate-900">{{ to_normal_date($data[0]->exit_date) }}</p></div>
                @isset($data[0]->observations)
                    <div class="rounded-lg bg-slate-50 p-4 lg:col-span-3"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Observations</p><p class="mt-1 text-slate-900">{{ $data[0]->observations }}</p></div>
                @endisset
            </div>
        </section>
        ...
        <section class="panel panel-pad">
            <div class="mb-5 flex items-center gap-3">
                <div class="flex h-11 w-11 items-center justify-center rounded-lg bg-slate-100 text-slate-700">
                    <svg class="h-6 w-6" viewBox="0 0 24 24"><path fill="currentColor" d="M12 3a9 9 0 1 0 9 9h-2a7 7 0 1 1-2.05-4.95L14 10h7V3l-2.63 2.63A8.96 8.96 0 0 0 12 3m-1 5v5l4.25 2.52l.77-1.28l-3.52-2.09V8z"/></svg>
                </div>
                <div>
                    <h2 class="text-lg font-semibold text-slate-900">Journal des changements</h2>
                    <p class="text-sm text-slate-500">Trace des modifications de cette visite.</p>
                </div>
            </div>

            @php
                $auditLabels = [
                    'visit_created_by_reception' => 'Creation par accueil',
                    'visit_created_existing_visitor' => 'Creation visiteur existant',
                    'orientation_updated' => 'Orientation modifiee',
                    'host_assigned_by_service' => 'Personne visitee affectee',
                    'visit_updated' => 'Visite modifiee',
                    'workflow_status_updated' => 'Statut modifie',
                    'visit_deleted' => 'Visite supprimee',
                ];
            @endphp
                    ...
                    @foreach($audits as $audit)
                        <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <p class="font-semibold text-slate-900">{{ $auditLabels[$audit->action] ?? $audit->action }}</p>
                                    <p class="text-xs text-slate-500">
                                        {{ $audit->changed_by_name ?? 'Utilisateur inconnu' }}
                                        @if($audit->profile_name) - {{ $audit->profile_name }} @endif
                                    </p>
                                </div>
                                <p class="text-xs font-semibold text-slate-500">{{ to_normal_date($audit->created_at) }}</p>
                            </div>

                            <div class="mt-3 grid gap-3 lg:grid-cols-2">
                                <div class="rounded-lg bg-white p-3">
                                    <p class="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Avant</p>
                                    @forelse($audit->old_values as $key => $value)
                                        <p class="text-sm text-slate-700"><span class="font-semibold">{{ $key }}:</span> {{ is_array($value) ? json_encode($value) : ($value ?? '-') }}</p>
                                    @empty
                                        <p class="text-sm text-slate-400">-</p>
                                    @endforelse
                                </div>
                                <div class="rounded-lg bg-white p-3">
                                    <p class="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Apres</p>
            ...
            @else
                <div class="rounded-lg bg-slate-50 p-4 text-sm font-medium text-slate-500">
                    Aucun changement enregistre pour cette visite.
                </div>
            @endif
        </section>
        @endif
